...
<td>
    <button 
        class="btn-small toggle-invite" 
        data-id="<?= $invite['id'] ?>" 
        data-status="<?= $invite['is_revoked'] ? 'revoked' : 'active' ?>"
    >
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.toggle-invite').forEach(button => {
        button.addEventListener('click', async () => {
            const id = button.dataset.id;
            const currentStatus = button.dataset.status;

            const response = await fetch('toggle_invite.php', {
                method: 'POST',
            ...
            if (data.success) {
                // Toggle status
                button.dataset.status = data.new_status === 'revoked' ? 'revoked' : 'active';
                button.textContent = data.new_status === 'revoked' ? '🔄 Restore' : '❌ Revoke';
                button.classList.toggle('revoked');
            } else {
                alert("Failed to toggle invite. Try again.");
            }
        });
    });
});
